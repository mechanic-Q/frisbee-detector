...
**下一步**（优先级序）：① auto_calibrate_v2 给 55-56min 段做段内标定→重跑矩阵使 G1/G4 可判；② 格④⑤ GPU 补跑；③ obj365 测试（已批，Part 2，许可纪律见对话记录：权重不进 git/数字标研究用/效果不好即删）；④ 负样本 bbox 实验 v3.5（挂起）。
...
__zcode_status=$?
if [ "$__zcode_status" -eq 0 ]; then pwd -P > '/c/Users/LMR/AppData/Local/Temp/zcode-36dcd1a4-d345-4364-9023-402cc8394d37-cwd'; fi
exit "$__zcode_status"
