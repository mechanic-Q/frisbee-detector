...
- [[golden-set-100-domain-validation]]：9 模型在真实转播画面 100 帧上的域外评测
- **方法论失败并修正**：自动金标准判据三层全失效（同源共识误检 / 标定仅单帧有效 / 白帽无法与飞盘区分）；
  人工巡检 210 共识框后判定真盘仅占 40-45%，此前发布的 mAP 排名作废
- **产出真价值**：域外误检类型清单——白帽子(~40%)、黄色场地锥、记分牌绿色装饰、橙色运动鞋、白色车身
- 结论转向：优先扩展负样本（非草地场景物体），而非换架构；事件统计瓶颈定位在飞盘检测
- 教训：同源模型共识≠真值；test 排名不可外推；无独立验证通道时宁可不给指标
