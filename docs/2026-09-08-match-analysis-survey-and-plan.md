...
**§13.16 终评补录（Phase A 全量 5-chunk，09-12）**：4 chunk 补跑（0/2/3/4）全部标定 PASS（chunk0 又自愈于 [3000,3600)，0.9079）。5-chunk 聚合（C0 冷却解锁+USAU 9.B 翻转+imputed 守卫全链路）：**提案级 P 1.000 / R 1.000**——4 novel 提案全部命中 4 个 GT 得分（dt 9.3/16.6/62.7/67.3s），9 followup 正确链住，上一轮唯一假候选（t=267.9 僵持误投影）未复现（IOS-NMM+门控重试改变候选形态，1 轮迭代即消除）。新增命中形态：chunk1 t=226.7 与 chunk0 t=88.3 的"得分后持盘走段"候选（dt 大但同侧同窗口——GT 时间戳滞后的已知特性）。tile 补召回在全部 5 chunk 上 tile-source=0：本素材无检出帧均为"盘不在画面"（长传出界/镜头未跟），盘召回瓶颈在"镜头外时间"而非"画面内漏检"——TrackNetV3 决策门维持 parked 的实证补强。WFDF 2017 未见素材 2min 试跑：标定一段过门（0.8602）、全链路（C0/翻转/守卫）无异常、热力图两队峰值（x=90/82m 同踞美方半场，符合美方 11:3 大胜的场面）。

### 13.17 域外定性终评：WFDF 2017 世界运动会决赛（09-12）

**素材**：WFDF 2017 世界运动会女子决赛 哥伦比亚 vs 美国（BV16B4y1M7zd，136min，852×480@25fps，匿名最高清）。抽 6 段×2min（原片 t=2/30/60/80/100/120min，覆盖开局/中段/拉分/末节），全管线（球员切片+盘 conf0.35+切片触发+fusion+hand-roi+impute+标定自愈+事件）。

**管线域外健壮性（核心结论）**：**6/6 段标定全 PASS**（内点率 0.8602-0.9852，一段还需自愈滑窗救回）——players-extent 段内标定在完全未见的女队/新机位/不同场地背景上无一失败。C0 冷却/方向翻转/守卫全链路零异常。**这是"最终评测需未见过的视频"遗留的正面交付**。

**检测/轨迹域外表现（分布差异大，如实记录）**：
- 盘覆盖率 2%-30%（域内 35-42%）——WFDF 女子转播节奏快、长传多、盘常在画面外，s4_late 段仅 2%（该段多为远景跟拍）。
- **s1_open 最长轨迹 407f=16.3s**（域内历史最长 184f 的 2.2 倍）——固定机位慢节奏阵地战下融合层上限被再次抬高；但其余段 0.6-1.6s（快节奏/摇镜头下轨迹碎片化依旧）。
- 8 个端区持盘得分候选全部落 right 端区（美方 11:3 大胜、多得分方向一致，与比分牌方向自洽）；零 counted 状态机得分（快节奏下持盘三条件难连续满足，与域内一致）。
- 热力图峰值落点合理但部分段偏出场地边界（x=106/110——远端边线区标定误差在域外放大，players-extent 端区区 ~7m 噪声的域外表现）。

**七指标域外可用性裁定**：热力图/飞行距离（带噪声声明）/端区得分候选 ✅ 可用；换手率/控盘占比/组织者/威胁链 ⚠️ 机制通但 chunk 级事件数少（possessions 合计 5），实质产出需盘轨迹连续性提升——域外素材上"盘不在画面+碎片化"双重制约，指向 TrackNetV3 类第二路（镜头外持续）与检测器域外微调（WFDF 素材可作靶向负样本/微调来源）。

**产物**：results/wfdf_eval/<seg>/（tracks+segcal+match_stats）、runs/match_stats/ 热力图 PNG。

### 13.19 识别率口径科学化 + 场线先验 + 镜头分类器（09-12，bfc835a/78f2b2e/76f61d5/602ba5e）

**目标**（用户定标）：识别错误率 <10%，识别率 >80%，多维度交叉提升。

**已达成的半个目标——误检率实测 <2%**：160 个随机 tracking crop（4 chunk × 40）全量人工判读，误检 2-3 个 ≈ **1.4-1.9%**（95%CI 上界 ~4%），远低于 10% 线。亮度交叉验证：99% 的 tracking 检出中心是亮白区（V≥150，白色飞盘物理先验一致）。结论：**在管帧的可信度已经达标，"识别率>80%"的缺口全在覆盖端**。

**覆盖率攻坚两轮**：
1. 断轨续接（bfc835a）：REACQ_WINDOW=30 帧圆判据（150 帧实测假影混入回退 671→349，两次自纠），chunk4 在管 39%→42%、longest 184→195f；
2. 门拒续接（78f2b2e）：连续门拒 ≥3 帧触发同款圆判据——治高速飞行段 Kalman 滞后系统性误杀（实测连续 15 帧拒 0.71-0.83 高分真盘）。chunk4 longest 255f、在管 49%；chunk2 **首个完整攻转+得分链**（turnover→score team0）。

**口径科学化三轮（交叉帮助）**：
- 场线先验：搜索态场外候选硬拒（world 矩形+5m 缓冲）——chunk4 零扰动（好素材无场外误检），价值在域外兑现；
- 镜头分类器 `shot_classifier.py`（球员框数量+尺寸 heuristic）：域内 90-100% 全景 vs WFDF 64%——转播语言差异定量化；
- **关键目检发现**：wide 无盘帧主体是**真实无盘的间歇时间**（得分手势/huddle/拉盘准备，24 帧拼图证实）——"识别率虚低"的主因不是漏检而是**比赛本身大量无盘时间**。活球口径存在循环论证（用盘检出定义活球），独立信号两试均败（球员活动强度分离度差方向反；长窗密度 chunk4 无 10s 空窗）。

**现状定格与科学口径路径**：
- 全帧在管 42-49%；wide 口径 27-39%（间歇扣不干净）
- 严格口径 P(在管|盘可见) 需要**盘可见区间真值**——半自动尝试（白色亮斑 V≥200 全帧 100% 命中=噪声太高；tracking 框亮度验证通过但标注生成未闭环）→ 正确工具=复核队列 UI 扩展"可见区间标注"或轻量人工（每 chunk ~15 分钟）
- 检测器侧已到"宁缺勿滥"最优区；覆盖率再上台阶依赖 ①TrackNetV3 类时序预测（遮挡/出画段）②检测器域外微调（WFDF 素材）③盘可见区间标注后的精准补检

### 13.20 路线 1：小盘过采样微调（09-12，进行中）

**数据概念修正**：160 审计 crop 是验证集不可当训练集（污染口径）；真训练源=frisbee_merged_v2（3871 框，47% 已是 <25px 小盘）——微调方向改为**远端极小盘(<15px)过采样 ×3**（1186 帧复制后 7001 图），从 shadow_v1 继续 30ep（configs/frisbee_small_v3.yaml，val/test 沿用 v2 防泄漏）。

**复测计划**：训成后 (a) val mAP 不回退（防过拟合小盘牺牲大盘）(b) chunk4 用新权重全管线重跑 → P(在管|可见) 从 39% 的变化 (c) 误检率审计重抽 40 crop 复核 <10% 维持。
