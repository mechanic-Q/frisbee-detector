...
### 13.7 v6 分支遗产合入与马氏门控复测（09-11，用户要求"进主线"）

背景：分支清理时确认 `archive/improve-precision-v6-mahalanobis`（2026-06-08）的"6 态 Kalman + 马氏门控"从未合入主线。调其 4 份实验文档复盘：**该实验当时没有做完**——v1/v2 两版门控因伤基线被回退，v3 改为"事后验证"设计后只做了 d² 数据采集（门控阈值停在 500=不拦截），端到端收益从未测量。
