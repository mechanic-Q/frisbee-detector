...
from tools.export_annotation_tasks import export_reviewed_tasks, xyxy_to_yolo_label
...
def test_xyxy_to_yolo_label_rejects_invalid_image_size():
    with pytest.raises(ValueError, match="image size"):
        xyxy_to_yolo_label([10.0, 20.0, 30.0, 40.0], (0, 100))

    with pytest.raises(ValueError, match="image size"):
        xyxy_to_yolo_label([10.0, 20.0, 30.0, 40.0], (100, -1))


def test_xyxy_to_yolo_label_rejects_out_of_bounds_bbox():
    with pytest.raises(ValueError, match="bbox invalid"):
        xyxy_to_yolo_label([-1.0, 20.0, 30.0, 40.0], (100, 100))

    with pytest.raises(ValueError, match="bbox invalid"):
        xyxy_to_yolo_label([10.0, 20.0, 101.0, 40.0], (100, 100))
...
def test_export_reviewed_tasks_rejects_non_empty_export_dir(tmp_path):
    ...
    export_dir = tmp_path / "export"
    export_dir.mkdir()
    (export_dir / "existing.txt").write_text("occupied", encoding="utf-8")
    write_tasks(
        task_store,
        [
            AnnotationTask(
    ...
    with pytest.raises(ValueError, match="export_dir.*not empty"):
        export_reviewed_tasks(task_store, export_dir, [], image_size=(100, 100))


def test_export_reviewed_tasks_rejects_duplicate_task_ids(tmp_path):
    frame = tmp_path / "frame.jpg"
    ...
    with pytest.raises(ValueError, match="duplicate"):
        export_reviewed_tasks(task_store, tmp_path / "export", [], image_size=(100, 100))


def test_export_report_counts_actual_written_files(tmp_path):
    frame = tmp_path / "assets" / "frame.jpg"
    crop = tmp_path / "assets" / "crop.jpg"
    frame.parent.mkdir()
    frame.write_bytes(b"frame")
    ...
    positive_images = list((export_dir / "images" / "positive").glob("*.jpg"))
    positive_labels = list((export_dir / "labels" / "positive").glob("*.txt"))
    hardneg_images = list((export_dir / "images" / "hard_negative").glob("*.jpg"))
    hardneg_labels = list((export_dir / "labels" / "hard_negative").glob("*.txt"))

    assert report["positive_count"] == len(positive_images) == len(positive_labels) == 1
    assert report["hard_negative_count"] == len(hardneg_images) == len(hardneg_labels) == 1
